...
def run_conversion(job_id, image_files, audio_path):
    try:
        ...
        print(f'[{job_id}] run_conversion starting. images = {image_files}, audio = {audio_path}')
        # Save images in order
        for f in image_files:
            if not os.path.exists(f) or os.path.getsize(f) == 0:
                raise RuntimeError(f"Input image missing or empty {f}")
        for i, f in enumerate(image_files, start=1):
            ext = Path(f.filename).suffix or '.jpg'
            target = workdir / f"img{i:03d}{uuid.uuid4().hex}{ext}"
            f.save(str(target))
